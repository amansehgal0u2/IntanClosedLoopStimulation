...
MainWindow::MainWindow(int sampleRateIndex_, int stimStepIndex)
{
    ...
    helpDialogSpikeBandFilter = nullptr;
...
void MainWindow::createLayout()
{
    ...
    helpDialogSpikeBandFilterButton = new QToolButton();
    helpDialogSpikeBandFilterButton->setText(tr("?"));
    helpDialogSpikeBandFilterButton->setToolButtonStyle(Qt::ToolButtonTextOnly);
    connect(helpDialogSpikeBandFilterButton, SIGNAL(clicked()), this, SLOT(spikeBandFilterHelp()));
...
//    sampleRateComboBox->setCurrentIndex(3);
    ...
    //spike band filter combo box
    spikeBandFilterComboBox = new QComboBox();
    spikeBandFilterComboBox->addItem(tr("Disabled"));
    spikeBandFilterComboBox->addItem(tr("Enabled"));
    spikeBandFilterComboBox->setCurrentIndex(0);
    ...
    connect(spikeBandFilterComboBox, SIGNAL(currentIndexChanged(int)),
            this, SLOT(changeSpikeBandFilter(int)));
...
//    bandwidthLayout->addWidget(lowerSettleBandwidthLabel);
    ...
    //spike band filter: label-comoboBox-helpButton
    QHBoxLayout *spikeBandFilterLayout = new QHBoxLayout;
    spikeBandFilterLayout->addWidget(new QLabel(tr("Spike Band Filter Status")));
    spikeBandFilterLayout->addWidget(spikeBandFilterComboBox);
    spikeBandFilterLayout->addStretch(1);
    spikeBandFilterLayout->addWidget(helpDialogSpikeBandFilterButton);
    ...
    offchipFilterLayout->addLayout(spikeBandFilterLayout);

    // add filter horizontal layout boxes
    QGroupBox *notchFilterGroupBox = new QGroupBox(tr("Software Filters"));
    notchFilterGroupBox->setLayout(offchipFilterLayout);
    notchFilterGroupBox->setLayout(spikeBandFilterLayout);
...
// Display the freq response and phase response of the spike band filter
void MainWindow::spikeBandFilterHelp()
{
    if (!helpDialogSpikeBandFilter)
    {
        helpDialogSpikeBandFilter = new HelpDialogSpikeBandFilter(this);
    }
    helpDialogSpikeBandFilter->show();
    helpDialogSpikeBandFilter->raise();
    helpDialogSpikeBandFilter->activateWindow();
    wavePlot->setFocus();
}
...
void MainWindow::changeSpikeBandFilter(int spikeBandFilterIndex)
{
    switch (spikeBandFilterIndex)
    {
        case 0:
            spikeBandFilterEnabled = false;
            break;
        case 1:
            spikeBandFilterEnabled = true;
            break;
    }
    // handle updates to the spike band filter
    signalProcessor->setSpikeBandFitlerEnabled(spikeBandFilterEnabled);
}
        ...
        if (newDataReady) {
            ...
            if(ClosedLoopStimEnabled > 0)
            {
                ...
                if(!signalProcessor->spikeDetectorCalibrated)
                {
                    ...
                    if ( retCode < 0 )
                    {
                        QMessageBox::critical(this,tr("Dynamic Memory Allocation Failure"),
                                                   tr("Spike Detector Calibration failed."
                                                      "Request for memory was denied. Application will exit."
                                                      "Re-start the applcation and try again. If the problem persists, "
                                                      "shut down the other running apps on your computer. If the problem still"
                                                      "persists then reduce the number of channels to run closed loop stimulation on"));
                        exit(EXIT_FAILURE); // abort application
                    }
...
// update the status bar text. Input text cannot be a null string
void MainWindow::updateStatusBarText(const QString& s)
{
    QString statusBarText = statusBarLabel->text();
    if( statusBarText != s )
    {
...
SignalProcessor* MainWindow::getSignalProcessorObj() const
{
...
void MainWindow::stimParam()
{
    ...
    if (displayDigOutButton->isChecked()) {  // Digital out channel
        ...
        stimParamDialog = new StimParamDialog(parameters, selectedChannel,
