...
// Enable/Disable the real time spike band filter
void SignalProcessor::setSpikeBandFitlerEnabled(bool enable)
{
    this->spikeBandFilterEnabled = enable;
}

// return the enable/disable status of the spike band filter
bool SignalProcessor::getSpikeBandFitlerStatus() const
{
    return this->spikeBandFilterEnabled;
}

// Update the sampling rate of the spike band filter
void SignalProcessor::setSpikeBandFitlerSamplingRate(unsigned int sr_idx)
{
    this->spikeBandFIRfilter->setSpikeBandFilterSamplingFreq(sr_idx);
}
                                 ...
                                 const QVector<QVector<bool> > &channelVisible)
{
    ...
    if(spikeBandFilterEnabled)
    {
        for (stream = 0; stream < numDataStreams; ++stream)
        {
            for(channel = 0; channel < CHANNELS_PER_STREAM; ++channel)
            {
                if (channelVisible.at(stream).at(channel))// apply filter only if channel is visible
                {
                    for (t = 0; t < length; ++t)
                    {
